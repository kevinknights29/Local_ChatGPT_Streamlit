...
def openai_key_validation():
    ...
    if "api_key_is_valid" not in st.session_state:
        if "openai" in st.secrets:
            if "api_key" in st.secrets.openai:
                ...
                st.warning(
                    "The OpenAI API Key provisioned is a invalid... Check secrets!",
                )
                st.session_state["api_key_is_valid"] = False
        else:
            st.warning("The OpenAI API Key provisioned is a invalid... Check secrets!")
            st.session_state["api_key_is_valid"] = False
    ...
    if not st.session_state["api_key_is_valid"]:
        st.warning("The OpenAI API Key provisioned is a invalid!")
        st.stop()
    st.success("The OpenAI API Key provisioned is a valid!")
